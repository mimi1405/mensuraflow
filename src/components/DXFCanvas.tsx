...
export function DXFCanvas({ onPointClick, onCursorMove, isPlacingCopy, copiedMeasurement, placementPosition, onDuplicate, onDelete }: DXFCanvasProps) {
  ...
  const renderCanvas = () => {
    ...
    if (measurements) {
      ...
      renderMeasurementLabels(ctx, measurements);
    }
  ...
  const renderMeasurementLabels = (ctx: CanvasRenderingContext2D, measurements: Measurement[]) => {
    ctx.save();
    ...
    const renderAreaLabel = (measurement: Measurement, centroid: Point, areaValue: number, label: string) => {
      ...
      ctx.fillText(areaText, 0, fontSize * 0.6);
    ...
    for (const measurement of measurements) {
      if (measurement.object_type === 'area' || measurement.object_type === 'window' || measurement.object_type === 'door') {
        if (measurement.floor_category) {
          if (
            (measurement.floor_category === 'ceiling' && !layerVisibility.ceiling) ||
            (measurement.floor_category === 'roomFloor' && !layerVisibility.roomFloor) ||
            (measurement.floor_category === 'finish' && !layerVisibility.finish)
          ) {
            continue;
          }
        }

        const centroid = calculateCentroid(measurement.geometry.points);

        const areaValue = measurement.area_m2 !== undefined
          ? measurement.area_m2
          : measurement.computed_value;

        const label = measurement.label || measurement.id.substring(0, 8);

        if (areaValue !== null && areaValue !== undefined) {
          renderAreaLabel(measurement, centroid, areaValue, label);
        }
      } else if (measurement.object_type === 'line' && measurement.geometry.points.length >= 2) {
        const lengthValue = measurement.computed_value;
        ...
        const label = measurement.label || measurement.id.substring(0, 8);
        const lengthText = `${lengthValue.toFixed(2)} m`;

        const start = measurement.geometry.points[0];
        const end = measurement.geometry.points[measurement.geometry.points.length - 1];
        ...
        const labelWidth = Math.max(
          ctx.measureText(label).width,
          ctx.measureText(lengthText).width
        );
        const padding = fontSize * 0.3;
        const bgWidth = labelWidth + padding * 2;
        const bgHeight = fontSize * 2.5;
        ...
        ctx.fillStyle = 'rgba(0, 0, 0, 0.9)';
        ctx.fillText(label, 0, -fontSize * 0.6);
        ctx.fillText(lengthText, 0, fontSize * 0.6);
